...
class _OrdersPageState extends State<OrdersPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null || user.id == null) {
      return Scaffold(body: Center(child: Text('Користувач не авторизований')));
    }
  ...
  Widget _buildOrderCard(int orderId, List<OrderDTO> orderItems) {
    ...
    Color statusColor;
    final isCancellable =
        firstOrder.state == 'В обробці' ||
        firstOrder.state == 'Не сплачено' ||
        firstOrder.state == "Сплачено";
    final canReorder =
        firstOrder.state == 'Скасовано' || firstOrder.state == 'Доставлено';
    ...
    final totalCost = orderItems.fold(
      0,
        ...
        children: [
          Row(
            children: [
              ...
              SizedBox(width: 24),
              Text('$totalCount товари', style: TextStyle(fontSize: 14)),
              SizedBox(width: 24),
              Text(
                '$totalCost ₴',
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            final orderItems = orders.where(
                              (o) => o.orderId == order.orderId,
                            );

                            for (var orderItem in orderItems) {
                              for (int i = 0; i < orderItem.count; i++) {
                                await cartProvider.addOrUpdateCartItem(
                                  orderItem.product,
                                  context,
                                );
                              }
